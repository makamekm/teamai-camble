...
- `test` — the action bound to `surfaces.chat.test`. It accepts exactly `test.rulet.tv` or `peprod.rulet.tv`, one or more of `Desktop`, `Chrome`, and `Android`, plus optional device, comment, and application/backend branch inputs. Repository branch bindings selected by the TeamAI chat scheduler are used when explicit branch inputs are absent. It resolves both source branches to full immutable SHAs before testing. Desktop and Chrome targets perform bounded HTTPS, HTML, real headless Chrome/Chromium DOM, and screenshot checks against `https://<environment>/?env=<environment>`. Android reuses the signed Gradle/preinit build path, verifies the APK with `apksigner`, stores one read-only full-SHA-named APK with SHA-256 provenance, and uses the installed `mobilerun` CLI to discover/ping the exact device, deliver/install that APK, exercise the same deep link in mobile Chrome, wait for the installed Camble package's app-ready signal, and capture both device screenshots.
...
The test response `output` is a machine-readable lifecycle with ordered step states, per-target states, exact Git provenance, artifact checksums/signature verification, screenshot records, bounded errors, and `verdict`/`verdictEvidence`. Missing Git access, signing credentials, `apksigner`, Chrome/Chromium, `mobilerun`, a fresh matching device, or any required page/app evidence is a terminal error. Error envelopes retain only already-produced bounded APK/screenshot artifacts, so partial failure evidence remains downloadable and the run never invents missing evidence.
...
Real Android chat tests additionally require `apksigner` from the configured Android SDK and a compatible installed `mobilerun` CLI/device. Desktop or Chrome targets require Google Chrome or Chromium on the selected agent.
...
The test suite injects a fake command runner and synthetic Git/Kubernetes/browser/Mobilerun fixtures. It covers reads, mutation planning, stale-SHA and exact test-input validation, browser/device command order, immutable APK provenance, terminal failures, redacted progress, special Camble ref rules, and dry-run non-mutation. It never contacts or mutates GitHub, Google Play, Kubernetes, a browser, or a device.
