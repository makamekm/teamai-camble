...
- `test` — the action bound to `surfaces.chat.test`. It accepts exactly `test.rulet.tv` or `peprod.rulet.tv` and only the `Android` target; the required test case is prefilled from the TeamAI chat problem. Repository branch bindings selected by the scheduler are used when explicit branch inputs are absent. It resolves both source branches to full immutable SHAs, applies a hashed test-only native-host overlay for the selected environment before and after Firebase Remote Config merges, builds one immutable signed APK, verifies that its compiled content contains only the selected backend, installs it through ADB, clears stale app data, pulls the installed `base.apk` back and requires its SHA-256 and backend evidence to match the built artifact, then launches the app and fail-closed verifies the effective `host`, `stage_host` and `stage` values only after a successful logcat buffer clear, using a PID-scoped release-build `TEAMAI_CHAT_TEST_RUNTIME_CONFIG` JSON marker before running `mobilerun run` with reasoning, vision, 80 steps and action-level trajectory capture. Static APK strings alone are not accepted as runtime backend proof. The prompt requires privacy/cookie and age gates, the existing-account login through Mobilerun secret IDs, the exact test case, all requested interaction assertions, and a final independently verifiable target screen. The final Mobilerun terminal event is authoritative: a failure reason is returned before credential-action completeness is evaluated, and a `Create a password`/unknown-account outcome is classified as an existing-account environment mismatch rather than masked by the intentionally absent password action. Failed runs retain bounded trajectory and screenshot hashes in the failed step while declaring no upload artifacts. The large transient APK is represented by SHA/signature/version/overlay/installed-artifact provenance but is not uploaded through TeamAI's bounded evidence channel.
