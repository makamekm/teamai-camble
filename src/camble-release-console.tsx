...
import { confirmedPlan } from "./confirmed-plan";
...
const isAgentFree = (agent: AgentSummary) => !Array.isArray(agent.activeJobs) || agent.activeJobs.length === 0;
...
function ActionAgent({ action, agents, value, onChange }: { action: PluginAction; agents: AgentSummary[]; value: string; onChange: (value: string) => void }) {
  const auto = agents.find(isAgentFree);
  return <label className="field camble-agent" data-action={action.id}><span>Агент</span><select value={value} onChange={(event) => onChange(event.target.value)}><option value="">{auto ? `Свободный агент (автовыбор: ${auto.name})` : "Нет свободного совместимого агента"}</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · {agent.platform}/{agent.arch}{isAgentFree(agent) ? " · свободен" : ` · занят (${agent.activeJobs?.length ?? 0})`}</option>)}</select></label>;
}
...
export function CambleReleaseConsole({ detail, agents, agentIds, runningAction, hasRunningOperation, canRun, supportsAction, onAgent, onRun }: Props) {
  ...
  const selectedAgent = (id: string) => agentIds[id] || compatible(id).find(isAgentFree)?.id || "";
  ...
  return <section className="camble-release-workspace">
    ...
    {environment && <div className="plugin-environment camble-tab-panel">
      ...
      {collectOperation ? <>
        <div className="plugin-snapshot-meta"><span><Clock3 /> {new Date(collectOperation.createdAt).toLocaleString("ru-RU")}</span><span><GitBranch /> {environment === "preprod" ? "dev → tags/*" : "tags/* → prod/*"}</span><span>{services.filter((service) => service.status === "stale").length} отличаются · {services.filter((service) => service.status === "current").length} актуально</span></div>
        <section className="plugin-selection camble-service-selection"><header><div><h4>Мультиселект сервисов</h4><p>По умолчанию ничего не выбрано. В обновление refs попадут только отмеченные сервисы.</p></div><div className="camble-selection-actions"><span>{selectedItems.length} выбрано</span><button type="button" onClick={() => setSelectedItems(availableServices.map((service) => service.id))}>Выделить все</button><button type="button" onClick={() => setSelectedItems([])}>Сбросить выделение</button></div></header><div className="plugin-items" role="group" aria-label={`${environment} мультиселект сервисов`}>{services.map((service) => <label key={`${service.repository}:${service.id}`} className={selectedItems.includes(service.id) ? "selected" : ""}><input type="checkbox" disabled={!service.sourceSha || busy("promote")} checked={selectedItems.includes(service.id)} onChange={(event) => toggle(service.id, event.target.checked)} /><span><PackageCheck /><strong>{service.id === "application3" ? "application" : service.id}</strong><small>{service.repository} · {service.sourceRef} → {service.targetRef}</small><small className={`camble-ref-status ${service.status}`}>{service.status === "current" ? "Актуальный" : service.status === "missing-source" ? "Нет Preprod ref" : environment === "preprod" ? "Отличается от dev" : "Отличается от Preprod"}</small><small>Текущий commit: <code>{shortSha(service.targetSha)}</code></small><small>Исходный commit: <code>{shortSha(service.sourceSha)}</code></small></span></label>)}</div></section>
        <div className="plugin-deploy-bar"><div><strong>{environment}: обновление Git refs</strong><p>Только подтверждённые SHA с lease; при изменении refs нужно новое подтверждение. CI/Devtron rollout и готовность приложения не проверяются.</p></div><div className="camble-deploy-controls"><ActionAgent action={actions.get("promote")!} agents={compatible("promote")} value={agentIds.promote ?? ""} onChange={(value) => onAgent("promote", value)} /><RunButton busy={busy("promote")} disabled={disabled("promote") || selectedItems.length === 0} onClick={() => void run("promote", { environment, items: selectedItems, "confirmed-plan": confirmedPlan(environment, selectedItems, services), "dry-run": false }, true)}>Обновить выбранные refs</RunButton></div></div>
